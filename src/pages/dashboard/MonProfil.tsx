...
import {
  ...
  ShieldCheck, FileCheck, Package, Globe, Clock,
} from 'lucide-react';
...
export default function MonProfil() {
  ...
  const uploadFile = async (file: File, folder: string): Promise<string | null> => {
    ...
    const { error } = await supabase.storage.from('stores').upload(fileName, file);
    if (error) return null;
    return supabase.storage.from('stores').getPublicUrl(fileName).data.publicUrl;
  };
  ...
  const handleFileUpload = async (
    ...
    if (url) onDone(url);
    else setError('Erreur lors de l\'upload du fichier.');
  };
  ...
  const handleMultiUpload = async (
    ...
    for (const file of files.slice(0, max - existing.length)) {
  ...
  const saveSection = useCallback(async (section: string, data: Partial<FormState>) => {
    ...
    setSaving(section);
    setError('');
    const updateData: Record<string, unknown> = { ...data, last_updated_at: new Date().toISOString() };
    updateData.profile_completion = calcCompletion({ ...form, ...data }, certs);

    const { data: result, error: err } = await supabase.from('producers').update(updateData).eq('id', producer.id).select('id');
    if (err) {
      setError(`Erreur: ${err.message}`);
    } else if (!result || result.length === 0) {
      setError('Impossible de sauvegarder (permission refusée). Vérifiez que votre profil producteur existe.');
    } else {
      await refresh();
      ...
      <div className="flex gap-6">
        ...
        <div className="flex-1 min-w-0 space-y-5">
          ...
          <SectionCard ref={el => { sectionRefs.current['1'] = el; }} data-section-id="1" id="1" title="Informations personnelles" icon={User} saved={savedSection === '1'} complete={sectionComplete('1', form, certs)}>
            <div>
              ...
              <div className="flex items-center gap-4">
                ...
                <input type="file" accept="image/*" ref={el => { fileRefs.current['avatar'] = el; }} onChange={e => handleFileUpload(e, 'avatar', url => update('avatar_url', url))} className="hidden" />
          ...
          <SectionCard ref={el => { sectionRefs.current['2'] = el; }} data-section-id="2" id="2" title="Pièces d'identité" icon={FileText} saved={savedSection === '2'} complete={sectionComplete('2', form, certs)}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <FileUploadField label="Recto (image ou PDF) *" fieldKey="id_recto" currentUrl={form.identity_recto_url} onUpload={url => update('identity_recto_url', url)} fileRefs={fileRefs} accept="image/*,.pdf" />
              <FileUploadField label="Verso (image ou PDF)" fieldKey="id_verso" currentUrl={form.identity_verso_url} onUpload={url => update('identity_verso_url', url)} fileRefs={fileRefs} accept="image/*,.pdf" />
            </div>
          ...
          <SectionCard ref={el => { sectionRefs.current['3'] = el; }} data-section-id="3" id="3" title="Organisation professionnelle" icon={Building2} saved={savedSection === '3'} complete={sectionComplete('3', form, certs)}>
            ...
            <SaveButton onClick={() => saveSection('3', { org_type: form.org_type, name: form.org_name || null, registration_number: form.registration_number || null, founded_year: form.founded_year ? parseInt(form.founded_year) : null, employee_count: form.employee_count ? parseInt(form.employee_count) : null, families_impacted: form.families_impacted ? parseInt(form.families_impacted) : null, website: form.website || null, business_email: form.business_email || null, short_description: form.short_description || null, long_description: form.long_description || null, story: form.long_description || null })} saving={saving === '3'} />
          </SectionCard>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pr-8">
                    ...
                    <div>
                      ...
                      <input type="file" accept=".pdf" onChange={async e => {
                        ...
                        const url = await uploadFile(file, `cert-${Date.now()}`);
                        if (url) updateCert(idx, 'document_url', url);
                      }} className="text-xs" />
                    </div>
                  </div>
                  <div className="mt-2">
          ...
          <SectionCard ref={el => { sectionRefs.current['7'] = el; }} data-section-id="7" id="7" title="Justificatifs et documents" icon={FileCheck} saved={savedSection === '7'} complete={sectionComplete('7', form, certs)}>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {BUSINESS_DOC_KEYS.map(doc => (
                <FileUploadField key={doc.key} label={`${doc.label} ${doc.required ? '*' : ''}`} fieldKey={`biz_${doc.key}`} currentUrl={form.business_documents[doc.key] ?? null}
                  onUpload={url => update('business_documents', { ...form.business_documents, [doc.key]: url })}
            ...
            <FileUploadField label="Analyses laboratoire des produits (PDF)" fieldKey="lab_analysis" currentUrl={form.lab_analysis_url}
              onUpload={url => update('lab_analysis_url', url)} fileRefs={fileRefs} accept=".pdf" />
            <FileUploadField label="Attestation d'absence de travail des enfants (PDF) *" fieldKey="ethical_charter_doc" currentUrl={form.ethical_charter_url ?? null}
              onUpload={url => update('ethical_charter_url', url)} fileRefs={fileRefs} accept=".pdf" />
            <div>
              ...
              <div className="grid grid-cols-5 gap-2">
                ...
                <input type="file" accept="image/*" multiple ref={el => { fileRefs.current['farm'] = el; }}
                  onChange={e => handleMultiUpload(e, 'farm', form.farm_photos, urls => update('farm_photos', urls), 10)} className="hidden" />
              </div>
            </div>
          ...
          <SectionCard ref={el => { sectionRefs.current['8'] = el; }} data-section-id="8" id="8" title="Logistique et livraison" icon={Truck} saved={savedSection === '8'} complete={sectionComplete('8', form, certs)}>
            ...
            <SaveButton onClick={() => saveSection('8', { delivery_countries: form.delivery_countries, transport_modes: form.transport_modes, delivery_days_avg: form.delivery_days_avg ? parseInt(form.delivery_days_avg) : null, packaging_types: form.packaging_types, has_insurance: form.has_insurance, shipping_paid_by: form.shipping_paid_by, logistics_partners: form.logistics_partners || null })} saving={saving === '8'} />
          </SectionCard>
          ...
          <SectionCard ref={el => { sectionRefs.current['10'] = el; }} data-section-id="10" id="10" title="Médias (photos/vidéos)" icon={ImageIcon} saved={savedSection === '10'} complete={sectionComplete('10', form, certs)}>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <FileUploadField label="Logo de l'organisation *" fieldKey="logo" currentUrl={form.logo_url} onUpload={url => update('logo_url', url)} fileRefs={fileRefs} accept="image/*" isImage />
              <div>
                ...
                <div className="relative h-32 rounded-2xl border-2 border-dashed border-gray-200 overflow-hidden bg-gray-50 flex items-center justify-center">
                  ...
                  <input type="file" accept="image/*" ref={el => { fileRefs.current['banner'] = el; }} onChange={e => handleFileUpload(e, 'banner', url => update('banner_url', url))} className="hidden" />
            ...
            <div>
              ...
              <div className="grid grid-cols-5 gap-2">
                ...
                <input type="file" accept="image/*" multiple ref={el => { fileRefs.current['team'] = el; }}
                  onChange={e => handleMultiUpload(e, 'team', form.team_photos, urls => update('team_photos', urls), 5)} className="hidden" />
              </div>
            </div>
            <div>
              ...
              <div className="grid grid-cols-5 gap-2">
                ...
                <input type="file" accept="image/*" multiple ref={el => { fileRefs.current['products'] = el; }}
                  onChange={e => handleMultiUpload(e, 'products', form.product_photos, urls => update('product_photos', urls), 10)} className="hidden" />
              </div>
            </div>
            <div>
...
function FileUploadField({ label, fieldKey, currentUrl, onUpload, fileRefs, accept, isImage }: {
  label: string;
  fieldKey: string;
      ...
      <div className="flex items-center gap-3">
        ...
        <input type="file" accept={accept} ref={el => { fileRefs.current[fieldKey] = el; }}
          onChange={async e => {
            ...
            const { error } = await supabase.storage.from('stores').upload(fileName, file);
            if (!error) onUpload(supabase.storage.from('stores').getPublicUrl(fileName).data.publicUrl);
          }} className="hidden" />
