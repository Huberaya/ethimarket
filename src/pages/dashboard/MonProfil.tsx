  ...
  if (!producer) {
    return (
      <div className="bg-white rounded-2xl border border-gray-100 p-12 text-center">
        ...
        <h3 className="font-bold text-gray-900 mb-2">{tx('Profil producteur non trouvé')}</h3>
        <p className="text-gray-500 text-sm">{tx('Votre profil n\'a pas encore été créé. Veuillez vous déconnecter puis reconnecter, ou contacter le support.')}</p>
      </div>
    );
  }
    ...
    <div className="max-w-6xl">
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            <h1 className="text-2xl font-black text-gray-900">{tx('Mon Profil Producteur')}</h1>
            <p className="text-gray-500 text-sm mt-1">{tx('Complétez votre profil pour créer la confiance avec les acheteurs professionnels')}</p>
          </div>
          {producer.slug && (
      ...
      <div className="flex gap-6">
        ...
        <div className="flex-1 min-w-0 space-y-5">
          ...
          <SectionCard ref={el => { sectionRefs.current['1'] = el; }} data-section-id="1" id="1" title={tx('Informations personnelles')} icon={User} saved={savedSection === '1'} complete={sectionComplete('1', form, certs)}>
            <FileUpload
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className={labelClass}>{tx('Téléphone principal')} <span className="text-red-500">*</span></label>
              ...
              <div>
                <label className={labelClass}>{tx('WhatsApp Business')}</label>
                <div className="flex gap-2">
                  <span className="px-3 py-2.5 bg-gray-100 rounded-xl text-sm text-gray-600 whitespace-nowrap">{getCountryDialCode(form.country)}</span>
            ...
            <div>
              <label className={labelClass}>{tx('Date de naissance')}</label>
              <input type="date" value={form.birth_date} onChange={e => update('birth_date', e.target.value)} className={inputClass} />
            </div>
            <div>
              <label className={labelClass}>{tx('Langues parlées')} <span className="text-red-500">*</span></label>
          ...
          <SectionCard ref={el => { sectionRefs.current['2'] = el; }} data-section-id="2" id="2" title={tx('Pièces d\'identité')} icon={FileText} saved={savedSection === '2'} complete={sectionComplete('2', form, certs)}>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className={labelClass}>{tx('Type de pièce')} <span className="text-red-500">*</span></label>
                <select value={form.identity_type} onChange={e => update('identity_type', e.target.value)} className={inputClass}>
                  {ID_TYPES.map(t => <option key={t.value} value={t.value}>{tx(t.label)}</option>)}
                </select>
              </div>
              <div>
                <label className={labelClass}>{tx('Numéro de la pièce')} <span className="text-red-500">*</span></label>
                <input type="text" value={form.identity_number} onChange={e => update('identity_number', e.target.value)} className={inputClass} />
              </div>
              <div>
                <label className={labelClass}>{tx('Pays émetteur')} <span className="text-red-500">*</span></label>
              ...
              <div>
                <label className={labelClass}>{tx('Date d\'expiration')} <span className="text-red-500">*</span></label>
                <input type="date" value={form.identity_expiry} onChange={e => update('identity_expiry', e.target.value)} className={inputClass} />
              </div>
            </div>
            <div>
              <label className={labelClass}>{tx('Date de délivrance')}</label>
          ...
          <SectionCard ref={el => { sectionRefs.current['3'] = el; }} data-section-id="3" id="3" title={tx('Organisation professionnelle')} icon={Building2} saved={savedSection === '3'} complete={sectionComplete('3', form, certs)}>
            <div>
              <label className={labelClass}>{tx('Type d\'organisation')} <span className="text-red-500">*</span></label>
              <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
                {ORG_TYPES.map(t => (
                  <button key={t.value} type="button" onClick={() => update('org_type', t.value)}
                    ...
                    <span className="text-xl">{t.icon}</span>{tx(t.label)}
                  </button>
                ))}
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className={labelClass}>{tx('Nom de l\'organisation')} <span className="text-red-500">*</span></label>
                <input type="text" value={form.org_name} onChange={e => update('org_name', e.target.value)} className={inputClass} />
              </div>
              <div>
                <label className={labelClass}>{tx('Numéro d\'enregistrement (SIRET, RCCM...)')}</label>
                <input type="text" value={form.registration_number} onChange={e => update('registration_number', e.target.value)} className={inputClass} />
              </div>
              <div>
                <label className={labelClass}>{tx('Date de création')} <span className="text-red-500">*</span></label>
              ...
              <div>
                <label className={labelClass}>{tx('Nombre d\'employés')} <span className="text-red-500">*</span></label>
              ...
              <div>
                <label className={labelClass}>{tx('Nombre de familles impactées (si coopérative)')}</label>
              ...
              <div>
                <label className={labelClass}>{tx('Site web')} <span className="text-gray-400 font-normal">(optionnel)</span></label>
                <input type="url" value={form.website} onChange={e => update('website', e.target.value)} placeholder="https://..." className={inputClass} />
              </div>
            </div>
            <div>
              <label className={labelClass}>{tx('Email professionnel de l\'organisation')}</label>
              <input type="email" value={form.business_email} onChange={e => update('business_email', e.target.value)} placeholder={tx('contact@coopérative.com')} className={inputClass} />
            </div>
            <div>
              <label className={labelClass}>{tx('Description courte')} <span className="text-red-500">*</span> <span className="text-gray-400 font-normal">{tx('(max 200 caractères)')}</span></label>
              <textarea rows={2} maxLength={200} value={form.short_description} onChange={e => update('short_description', e.target.value)} placeholder={tx('Une phrase qui résume votre activité')} className={`${inputClass} resize-none`} />
              <p className="text-xs text-gray-500 mt-1">{form.short_description.length}/200</p>
            </div>
            <div>
              <label className={labelClass}>{tx('Description longue / Histoire')} <span className="text-red-500">*</span> <span className="text-gray-400 font-normal">{tx('(min 500 caractères)')}</span></label>
          ...
          <SectionCard ref={el => { sectionRefs.current['4'] = el; }} data-section-id="4" id="4" title={tx('Localisation')} icon={MapPin} saved={savedSection === '4'} complete={sectionComplete('4', form, certs)}>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className={labelClass}>{tx('Pays')} <span className="text-red-500">*</span></label>
                <select value={form.country} onChange={e => update('country', e.target.value)} className={inputClass}>
                  {COUNTRIES.map(c => <option key={c.code}>{c.name}</option>)}
                </select>
              </div>
              <div>
                <label className={labelClass}>{tx('Région / Province')} <span className="text-red-500">*</span></label>
                <input type="text" value={form.region} onChange={e => update('region', e.target.value)} className={inputClass} />
              </div>
              <div>
                <label className={labelClass}>{tx('Ville')} <span className="text-red-500">*</span></label>
                <input type="text" value={form.city} onChange={e => update('city', e.target.value)} className={inputClass} />
              </div>
              <div>
                <label className={labelClass}>{tx('Code postal')}</label>
                <input type="text" value={form.postal_code} onChange={e => update('postal_code', e.target.value)} className={inputClass} />
              </div>
            </div>
            <div>
              <label className={labelClass}>{tx('Adresse complète')} <span className="text-red-500">*</span></label>
              <textarea rows={3} value={form.address} onChange={e => update('address', e.target.value)} className={`${inputClass} resize-none`} />
            </div>
            <div>
              <label className={labelClass}>{tx('Point de repère')} <span className="text-gray-400 font-normal">(optionnel)</span></label>
              <input type="text" value={form.landmark} onChange={e => update('landmark', e.target.value)} placeholder={tx('Ex: près de la rivière...')} className={inputClass} />
            </div>
            <div>
              <label className={labelClass}>{tx('Coordonnées GPS')} <span className="text-red-500">*</span></label>
              <div className="flex items-center gap-2">
                <input type="text" value={form.latitude} onChange={e => update('latitude', e.target.value)} placeholder={tx('Latitude')} className={`${inputClass} flex-1`} />
                <input type="text" value={form.longitude} onChange={e => update('longitude', e.target.value)} placeholder={tx('Longitude')} className={`${inputClass} flex-1`} />
            ...
            <p className="text-xs text-gray-500 bg-gray-50 rounded-xl p-3">{tx('Ces coordonnées seront visibles publiquement sur les fiches produits pour garantir la traçabilité.')}</p>
          ...
          <SectionCard ref={el => { sectionRefs.current['5'] = el; }} data-section-id="5" id="5" title={tx('Production')} icon={Sprout} saved={savedSection === '5'} complete={sectionComplete('5', form, certs)}>
            <div>
              <label className={labelClass}>{tx('Types de produits')} <span className="text-red-500">*</span></label>
              <div className="flex flex-wrap gap-2">
                {PRODUCT_TYPES.map(t => (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className={labelClass}>{tx('Surface cultivée')} <span className="text-red-500">*</span></label>
                <div className="flex gap-2">
              ...
              <div>
                <label className={labelClass}>{tx('Capacité de production annuelle')} <span className="text-red-500">*</span></label>
                <div className="flex gap-2">
                  ...
                  <select value={form.capacity_unit} onChange={e => update('capacity_unit', e.target.value)} className={`${inputClass} w-32`}>
                    <option>kg</option><option>tonnes</option><option>litres</option><option>{tx('pièces')}</option>
                  </select>
                </div>
              </div>
            </div>
            <div>
              <label className={labelClass}>{tx('Rendement moyen')} <span className="text-gray-400 font-normal">(optionnel)</span></label>
            ...
            <div>
              <label className={labelClass}>{tx('Méthodes de culture')} <span className="text-red-500">*</span></label>
              <div className="flex flex-wrap gap-2">
                {FARMING_METHODS.map(m => (
                  <button key={m.label} type="button" onClick={() => toggleArray('farming_methods', m.label)}
                    ...
                    <span>{m.icon}</span>{tx(m.label)}
                  </button>
                ))}
              </div>
            </div>
            <div>
              <label className={labelClass}>{tx('Techniques utilisées')}</label>
              <textarea rows={3} value={form.techniques_description} onChange={e => update('techniques_description', e.target.value)} placeholder={tx('Décrivez vos techniques : irrigation, protection des cultures, récolte, etc.')} className={`${inputClass} resize-none`} />
            </div>
            <div>
              <label className={labelClass}>{tx('Saisonnalité de production')} <span className="text-red-500">*</span></label>
            ...
            <div>
              <label className={labelClass}>{tx('Volume disponible actuellement')}</label>
          ...
          <SectionCard ref={el => { sectionRefs.current['6'] = el; }} data-section-id="6" id="6" title={tx('Certifications')} icon={Award} saved={savedSection === 'certs'} complete={sectionComplete('6', form, certs)}>
              ...
              {certs.map((cert, idx) => (
                <div key={idx} className="bg-gray-50 rounded-2xl p-4 border border-gray-100 relative">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pr-8">
                    <div>
                      <label className="text-xs font-semibold text-gray-600 mb-1 block">{tx('Type de certification')} <span className="text-red-500">*</span></label>
                    ...
                    <div>
                      <label className="text-xs font-semibold text-gray-600 mb-1 block">{tx('Numéro')} <span className="text-red-500">*</span></label>
                      <input type="text" value={cert.cert_number} onChange={e => updateCert(idx, 'cert_number', e.target.value)} className={inputClass} />
                    </div>
                    <div>
                      <label className="text-xs font-semibold text-gray-600 mb-1 block">{tx('Organisme certificateur')} <span className="text-red-500">*</span></label>
                      <input type="text" value={cert.cert_body} onChange={e => updateCert(idx, 'cert_body', e.target.value)} className={inputClass} />
                    </div>
                    <div>
                      <label className="text-xs font-semibold text-gray-600 mb-1 block">{tx('Date d\'obtention')} <span className="text-red-500">*</span></label>
                      <input type="date" value={cert.issue_date} onChange={e => updateCert(idx, 'issue_date', e.target.value)} className={inputClass} />
                    </div>
                    <div>
                      <label className="text-xs font-semibold text-gray-600 mb-1 block">{tx('Date d\'expiration')} <span className="text-red-500">*</span></label>
          ...
          <SectionCard ref={el => { sectionRefs.current['7'] = el; }} data-section-id="7" id="7" title={tx('Justificatifs et documents')} icon={FileCheck} saved={savedSection === '7'} complete={sectionComplete('7', form, certs)}>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {BUSINESS_DOC_KEYS.map(doc => (
                <FileUpload
                  ...
                  accept=".pdf,.jpg,.jpeg,.png"
                  maxSizeMB={10}
                  label={`${tx(doc.label)} ${doc.required ? '*' : ''}`}
                  description="Format PDF ou image, max 10 MB"
          ...
          <SectionCard ref={el => { sectionRefs.current['8'] = el; }} data-section-id="8" id="8" title={tx('Logistique et livraison')} icon={Truck} saved={savedSection === '8'} complete={sectionComplete('8', form, certs)}>
            <div>
              <label className={labelClass}>{tx('Pays vers lesquels vous livrez')} <span className="text-red-500">*</span></label>
              <div className="flex gap-2 mb-2">
                <select onChange={e => { if (e.target.value === '__all__') { update('delivery_countries', COUNTRIES.map(c => c.name)); } else if (e.target.value) toggleArray('delivery_countries', e.target.value); e.target.value = ''; }} className={inputClass}>
                  ...
                  <option value="__all__">{tx('Tous les pays')}</option>
            ...
            <div>
              <label className={labelClass}>{tx('Moyens de transport')} <span className="text-red-500">*</span></label>
              <div className="flex flex-wrap gap-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className={labelClass}>{tx('Délai moyen de livraison (jours)')} <span className="text-red-500">*</span></label>
              ...
              <div>
                <label className={labelClass}>{tx('Assurance transport incluse')} <span className="text-red-500">*</span></label>
                <div className="flex gap-2">
                  <button type="button" onClick={() => update('has_insurance', true)} className={`flex-1 py-2.5 rounded-xl text-sm font-semibold border-2 transition-all ${form.has_insurance ? 'border-brand-500 bg-brand-50 text-brand-700' : 'border-gray-200 text-gray-600'}`}>{tx('Oui')}</button>
                  <button type="button" onClick={() => update('has_insurance', false)} className={`flex-1 py-2.5 rounded-xl text-sm font-semibold border-2 transition-all ${!form.has_insurance ? 'border-brand-500 bg-brand-50 text-brand-700' : 'border-gray-200 text-gray-600'}`}>{tx('Non')}</button>
                </div>
              </div>
            </div>
            <div>
              <label className={labelClass}>{tx('Coût livraison à charge de')}</label>
              <select value={form.shipping_paid_by} onChange={e => update('shipping_paid_by', e.target.value)} className={inputClass}>
                <option value="producteur">{tx('Producteur')}</option>
                <option value="acheteur">{tx('Acheteur')}</option>
                <option value="partage">{tx('Partagé')}</option>
              </select>
            </div>
            <div>
              <label className={labelClass}>{tx('Type d\'emballage utilisé')} <span className="text-red-500">*</span></label>
              <div className="flex flex-wrap gap-2">
                {PACKAGING_TYPES.map(p => (
                  <button key={p.label} type="button" onClick={() => toggleArray('packaging_types', p.label)}
                    ...
                    <span>{p.icon}</span>{tx(p.label)}
                  </button>
                ))}
              </div>
            </div>
            <div>
              <label className={labelClass}>{tx('Partenaires logistiques')} <span className="text-gray-400 font-normal">(optionnel)</span></label>
          ...
          <SectionCard ref={el => { sectionRefs.current['9'] = el; }} data-section-id="9" id="9" title={tx('Engagement éthique')} icon={Heart} saved={savedSection === '9'} complete={sectionComplete('9', form, certs)}>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className={labelClass}>{tx('Employés à temps plein')} <span className="text-red-500">*</span></label>
              ...
              <div>
                <label className={labelClass}>{tx('Employés à temps partiel')}</label>
              ...
              <div>
                <label className={labelClass}>{tx('Salaire minimum garanti')} <span className="text-red-500">*</span></label>
                <div className="flex gap-2">
              ...
              <div>
                <label className={labelClass}>{tx('Heures de travail par semaine')} <span className="text-red-500">*</span></label>
              ...
              <div>
                <label className={labelClass}>{tx('Congés payés')}</label>
                <select value={form.paid_leave} onChange={e => update('paid_leave', e.target.value)} className={inputClass}>
                  <option value="">{tx('Sélectionner...')}</option>
                  <option value="oui">{tx('Oui')}</option>
                  <option value="non">{tx('Non')}</option>
                  <option value="details">{tx('Oui, avec détails')}</option>
                </select>
              </div>
              <div>
                <label className={labelClass}>{tx('Sécurité sociale / assurance maladie')}</label>
                <div className="flex gap-2">
                  <button type="button" onClick={() => update('health_insurance', true)} className={`flex-1 py-2.5 rounded-xl text-sm font-semibold border-2 transition-all ${form.health_insurance ? 'border-brand-500 bg-brand-50 text-brand-700' : 'border-gray-200 text-gray-600'}`}>{tx('Oui')}</button>
                  <button type="button" onClick={() => update('health_insurance', false)} className={`flex-1 py-2.5 rounded-xl text-sm font-semibold border-2 transition-all ${!form.health_insurance ? 'border-brand-500 bg-brand-50 text-brand-700' : 'border-gray-200 text-gray-600'}`}>{tx('Non')}</button>
                </div>
              </div>
            </div>
            <div>
              <label className={labelClass}>{tx('Conditions de travail (min 200 caractères)')} <span className="text-red-500">*</span></label>
              <textarea rows={3} value={form.working_conditions} onChange={e => update('working_conditions', e.target.value)} placeholder={tx('Décrivez les conditions de travail : sécurité, formation, avantages, etc.')} className={`${inputClass} resize-none`} />
              <p className="text-xs text-gray-500 mt-1">{form.working_conditions.length} caractères</p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
              <div>
                <label className={labelClass}>{tx('CO2 économisé/an (tonnes)')}</label>
                <input type="text" value={form.co2_saved} onChange={e => update('co2_saved', e.target.value)} className={inputClass} />
              </div>
              <div>
                <label className={labelClass}>{tx('Eau économisée/an (litres)')}</label>
                <input type="text" value={form.water_saved} onChange={e => update('water_saved', e.target.value)} className={inputClass} />
              </div>
              <div>
                <label className={labelClass}>{tx('Arbres préservés')}</label>
                <input type="text" value={form.trees_preserved} onChange={e => update('trees_preserved', e.target.value)} className={inputClass} />
              </div>
              <div>
                <label className={labelClass}>{tx('Superficie protégée (ha)')}</label>
                <input type="text" value={form.protected_area} onChange={e => update('protected_area', e.target.value)} className={inputClass} />
              </div>
            </div>
            <div>
              <label className={labelClass}>{tx('Actions communautaires')}</label>
              <textarea rows={3} value={form.social_actions} onChange={e => update('social_actions', e.target.value)} placeholder={tx('Décrivez vos actions : école, santé, formation, infrastructure, etc.')} className={`${inputClass} resize-none`} />
            </div>
            <div className="bg-brand-50 rounded-2xl p-4 border border-brand-100">
              <h4 className="font-bold text-brand-800 text-sm mb-2">{tx('Charte éthique EthiMarket')}</h4>
          ...
          {/* SECTION 10 — Médias */}
          <SectionCard ref={el => { sectionRefs.current['10'] = el; }} data-section-id="10" id="10" title={tx('Médias (photos/vidéos)')} icon={ImageIcon} saved={savedSection === '10'} complete={sectionComplete('10', form, certs)}>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <FileUpload
            ...
            <div>
              <label className={labelClass}>{tx('Vidéo de présentation')} <span className="text-gray-400 font-normal">(optionnel)</span></label>
              ...
              <p className="text-xs text-gray-500 mt-1">{tx('Durée conseillée : 2-3 minutes')}</p>
            </div>
    ...
    <div ref={ref} {...rest} className="bg-white rounded-2xl border border-gray-100 p-6 shadow-card scroll-mt-20">
      <div className="flex items-center gap-3 mb-5 pb-4 border-b border-gray-100">
        ...
        <div>
          ...
          {saved && <p className="text-xs text-brand-600 font-semibold flex items-center gap-1 mt-0.5"><CheckCircle className="w-3 h-3" /> {tx('Enregistré')}</p>}
        </div>
