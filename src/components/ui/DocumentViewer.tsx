...
export function DocumentViewer({
  ...
  const isImage = url
    ? url.match(/\.(jpeg|jpg|gif|png|webp)($|\?)/i) || url.startsWith('data:image/') || docType === 'image'
    : false;
  const isPDF = url
    ? url.toLowerCase().endsWith('.pdf') || url.startsWith('data:application/pdf') || docType === 'pdf'
    : false;
  ...
  const handleStatus = (newStatus: 'valid' | 'invalid') => {
    ...
    if (newStatus === 'valid' && onApprove) onApprove();
    if (newStatus === 'invalid' && onReject) onReject(localComment);
  };
  ...
  const handleDownload = () => {
    ...
    if (url.startsWith('data:')) {
      ...
      a.href = url;
      a.download = title.toLowerCase().replace(/\s+/g, '_') || 'document';
    ...
    >
      <div className="flex items-start justify-between gap-3 mb-3">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">
            {url ? (isImage ? 'Fichier image' : isPDF ? 'Fichier PDF' : 'Document officiel') : 'Non téléversé'}
          </p>
        </div>
      ...
      {url ? (
        <div className="space-y-3">
            ...
            <div className="p-3.5 bg-gray-50 border border-gray-200 rounded-xl flex items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                <button
                  type="button"
                  onClick={() => setModalOpen(true)}
                  className="px-3 py-1.5 bg-white border border-gray-200 hover:bg-gray-50 text-gray-700 font-semibold text-xs rounded-lg flex items-center gap-1.5 shadow-2xs"
                >
                  <Eye className="w-3.5 h-3.5 text-emerald-600" /> Voir
                </button>
                <button
                  type="button"
                  onClick={handleDownload}
          ...
          {(onStatusChange || onApprove || onReject) && (
      ...
      {modalOpen && url && (
        ...
        >
          <div
            className="relative w-full max-w-5xl max-h-[92vh] bg-white rounded-2xl overflow-hidden shadow-2xl flex flex-col"
          ...
          >
            <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100 bg-gray-50/70">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  type="button"
                  onClick={() => setModalOpen(false)}
                  className="p-1.5 text-gray-500 hover:text-gray-900 rounded-lg hover:bg-gray-200"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
            ...
            <div className="p-4 flex-1 overflow-auto bg-gray-900/5 flex items-center justify-center min-h-[400px]">
              {isImage ? (
                <img
                  src={url}
                  alt={title}
                  style={{ transform: `rotate(${rotation}deg)` }}
                  className="max-h-[75vh] w-auto object-contain rounded-lg shadow-sm transition-transform duration-200"
                />
              ) : (
                <iframe
                  src={url}
