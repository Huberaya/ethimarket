    ...
    <>
      ...
      <div className="hidden lg:block sticky bottom-0 z-30 bg-white/95 backdrop-blur-md border-t border-gray-100 -mx-4 sm:-mx-6 lg:-mx-8 px-4 sm:px-6 lg:px-8 py-3">
        <div className="max-w-7xl mx-auto flex items-center gap-3">
          <div className="flex-1 min-w-0">
            <p className="text-xs text-gray-500">{t('pd.orderAllIncluded')}</p>
            <p className="text-lg font-black text-gray-900 truncate">{total === null ? t('pd.onQuote') : `${total.toLocaleString('fr-FR')} €`} <span className="text-sm font-medium text-gray-400">{total === null ? '' : t('pd.plusShipping')}</span></p>
          </div>
          <div className="flex gap-2">
            <button onClick={onOrder} className="btn-primary px-5 py-2.5 text-sm inline-flex items-center gap-2">
              <ShoppingCart className="w-4 h-4" /> {t('pd.order')}
            </button>
            <button className="btn-outline px-3.5 py-2.5 text-sm inline-flex items-center gap-2">
              <MessageSquare className="w-4 h-4" /> <span className="hidden xl:inline">{t('pd.contact')}</span>
            </button>
            <button aria-label={t('pd.documents')} className="btn-outline px-3.5 py-2.5 text-sm inline-flex items-center gap-2">
              <Download className="w-4 h-4" /> <span className="hidden xl:inline">{t('pd.documents')}</span>
            </button>
            <button onClick={() => setFavorited(f => !f)} aria-label={favorited ? t('pd.removeFavorite') : t('pd.addFavorite')} aria-pressed={favorited} className={`btn-outline px-3.5 py-2.5 ${favorited ? 'text-red-500 border-red-200 bg-red-50' : ''}`}>
              <Heart className={`w-4 h-4 ${favorited ? 'fill-red-500' : ''}`} />
            </button>
            <button onClick={() => setShareOpen(true)} aria-label={t('pd.share')} className="btn-outline px-3.5 py-2.5">
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-30 bg-white/95 backdrop-blur-md border-t border-gray-100 px-4 py-3">
        <div className="flex items-center gap-2">
          <button onClick={() => setFavorited(f => !f)} aria-label={favorited ? t('pd.removeFavorite') : t('pd.addFavorite')} aria-pressed={favorited} className={`w-11 h-11 rounded-xl border-2 flex items-center justify-center flex-shrink-0 ${favorited ? 'border-red-200 text-red-500 bg-red-50' : 'border-gray-200 text-gray-500'}`}>
          ...
          <button onClick={onOrder} className="btn-primary flex-1 py-3 text-sm inline-flex items-center justify-center gap-2">
            <ShoppingCart className="w-4 h-4" /> {t('pd.order')} — {total === null ? t('pd.onQuote') : `${total.toLocaleString('fr-FR')} €`}
          </button>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShareOpen(false)}>
          <div className="bg-white rounded-3xl max-w-sm w-full p-6" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-black text-gray-900">{t('pd.share')}</h3>
            ...
            />
            <p className="text-xs text-gray-500 mt-2">{t('pd.clickToCopy')}</p>
          </div>
        </div>
      )}
    </>
