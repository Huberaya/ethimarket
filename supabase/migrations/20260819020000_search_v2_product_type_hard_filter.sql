-- =============================================================
-- EthiMarket Search V2 — Correctif précision : p_product_types et
-- p_materials étaient déclarés mais jamais appliqués dans le WHERE.
-- Conséquence : "café bio" renvoyait TOUS les produits bio.
-- Le type de produit devient un filtre dur (nom, product_type,
-- category_tags, keywords via synonymes côté client).
-- =============================================================
...
BEGIN
  ...
  WITH filtered_candidates AS (
    ...
    WHERE 
      ...
      AND (p_product_types IS NULL OR EXISTS (
             SELECT 1 FROM unnest(p_product_types) pt
             WHERE lower(unaccent(p.name)) LIKE '%' || lower(unaccent(pt)) || '%'
                OR lower(unaccent(coalesce(p.product_type, ''))) LIKE '%' || lower(unaccent(pt)) || '%'
                OR EXISTS (SELECT 1 FROM unnest(coalesce(p.category_tags, '{}')) ct
                           WHERE lower(unaccent(ct)) LIKE '%' || lower(unaccent(pt)) || '%')
                OR EXISTS (SELECT 1 FROM unnest(coalesce(p.keywords, '{}')) kw
                           WHERE lower(unaccent(kw)) LIKE '%' || lower(unaccent(pt)) || '%')))
      AND (p_materials IS NULL OR (
             SELECT bool_and(
               lower(unaccent(p.name || ' ' || coalesce(p.description, '') || ' ' || coalesce(p.short_description, ''))) LIKE '%' || lower(unaccent(m)) || '%'
               OR EXISTS (SELECT 1 FROM jsonb_array_elements_text(coalesce(p.attributes->'materials', '[]'::jsonb)) am
                          WHERE lower(unaccent(am)) LIKE '%' || lower(unaccent(m)) || '%'))
             FROM unnest(p_materials) m))
